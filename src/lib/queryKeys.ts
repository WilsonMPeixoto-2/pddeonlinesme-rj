...
export type DocumentGenerationRun = Database["public"]["Tables"]["document_generation_runs"]["Row"];

export type UnidadeLocalizador = Tables<"vw_unidades_localizador"> & { id: string; designacao: string };
export type DashboardUnidadeResumo = Tables<"vw_unidades_localizador"> & { id: string; designacao: string };
...
export interface FinanceiroUnidadeData {
...
export const queryKeys = {
  dashboardBasico: (exercicio: number, programa: string) => ["dashboard-basico", exercicio, programa] as const,
  dashboardUnidadesResumo: () => ["dashboard-unidades-resumo"] as const,
  unidadesDetalheLista: (exercicio: number, programa: string) => ["unidades-detalhe-lista", exercicio, programa] as const,
...
function toRepasseFinanceiro(row: Tables<"vw_repasses_financeiros_unidade">): RepasseFinanceiro | null {
  ...
  return {
    ...
    valor_programado: row.valor_programado,
    valor_pago: row.valor_pago,
    data_pagamento: row.data_pagamento,
    data_ordem_pagamento: row.data_ordem_pagamento,
    conta_bancaria_id: row.conta_bancaria_id,
    banco: row.banco,
    agencia: row.agencia,
    conta_corrente: row.conta_corrente,
    custeio_programado: row.custeio_programado,
    capital_programado: row.capital_programado,
    custeio_pago: row.custeio_pago,
    capital_pago: row.capital_pago,
  };
}

export const dashboardBasicoOptions = (exercicio: number, programa: string) => queryOptions<DashboardBasico | null, Error>({
  queryKey: queryKeys.dashboardBasico(exercicio, programa),
  enabled: Number.isFinite(exercicio) && Boolean(programa),
  queryFn: async () => {
    const { data, error } = await supabase.from("vw_dashboard_basico").select("exercicio, programa, total_unidades, total_reprogramado_custeio, total_reprogramado_capital, total_reprogramado, total_parcela_1_custeio, total_parcela_1_capital, total_parcela_2_custeio, total_parcela_2_capital, total_parcelas, total_disponivel_inicial, updated_at_max").eq("exercicio", exercicio).eq("programa", programa).maybeSingle();
...
export const unidadesDetalheListaOptions = (exercicio: number, programa: string) => queryOptions<UnidadeDetalhe[], Error>({
  ...
  enabled: Number.isFinite(exercicio) && Boolean(programa),
  queryFn: async () => {
    const { data, error } = await supabase.from("vw_unidade_detalhe").select(UNIDADE_DETALHE_COLUMNS).eq("exercicio", exercicio).eq("programa", programa).order("designacao", { ascending: true });
    if (error) throw new Error(error.message);
    return (data as UnidadeDetalhe[] | null) ?? [];
  },
});

export const unidadeDetalheOptions = (unidadeId: string | undefined, exercicio: number, programa: string) => queryOptions<UnidadeDetalhe | null, Error>({
  ...
  queryFn: async () => {
    const { data, error } = await supabase.from("vw_unidade_detalhe").select(UNIDADE_DETALHE_COLUMNS).eq("unidade_id", unidadeId!).eq("exercicio", exercicio).eq("programa", programa).maybeSingle();
    if (error) throw new Error(error.message);
    return data ?? null;
  },
});

export const unidadesLocalizadorOptions = () => queryOptions<UnidadeLocalizador[], Error>({
  queryKey: queryKeys.unidadesLocalizador(),
  queryFn: async () => {
    const { data, error } = await supabase.from("vw_unidades_localizador").select("id, designacao, nome, inep, cnpj, diretor").order("designacao");
    if (error) throw new Error(error.message);
    return (data ?? []).filter((u): u is UnidadeLocalizador => u.id !== null && u.designacao !== null);
  },
});
...
export const documentGenerationRunsOptions = ({ limit = 10, page = 1, status, exercicio }: DocumentGenerationRunsParams = {}) => queryOptions<{ runs: DocumentGenerationRun[]; count: number }, Error>({
  queryKey: queryKeys.documentGenerationRuns.list(limit, page, status, exercicio),
  queryFn: async () => {
    let query = supabase.from("document_generation_runs").select("*", { count: "exact" }).order("started_at", { ascending: false });
    if (status && status !== "all") query = query.eq("status", status);
    if (exercicio && exercicio !== "all") {
      const exVal = typeof exercicio === "string" ? parseInt(exercicio, 10) : exercicio;
      if (!isNaN(exVal)) query = query.eq("exercicio", exVal);
    }
    const from = (page - 1) * limit;
    query = query.range(from, from + limit - 1);
    const { data, error, count } = await query;
    if (error) throw new Error(error.message);
    return { runs: (data as DocumentGenerationRun[] | null) ?? [], count: count ?? 0 };
  },
});

export const adminUsersOptions = () => queryOptions<AdminUserRow[], Error>({
  ...
  queryFn: async () => {
    ...
    return data.map((row) => {
      const r = row as { user_id: string; email: string; created_at: string; last_sign_in_at: string | null; email_confirmed_at: string | null; roles: string[] | null };
      return { user_id: r.user_id, email: r.email, created_at: r.created_at, last_sign_in_at: r.last_sign_in_at, email_confirmed_at: r.email_confirmed_at, roles: (r.roles ?? []).filter((role): role is AppRole => role === "admin" || role === "operador") };
    });
  },
});
