...
> **Portal operacional e governança documental do Programa Dinheiro Direto na Escola (PDDE)** para a 4ª Coordenadoria Regional de Educação da Secretaria Municipal de Educação do Rio de Janeiro (SME-RJ).
...
O **PDDE Online** centraliza a gestão de saldos, execução financeira e conformidade de prestação de contas das unidades escolares sob jurisdição da 4ª CRE. O sistema opera como um **painel operacional de governança**, projetado com foco em eliminação de erros, automação e experiência do usuário otimizada.

### 🚀 Funcionalidades Implementadas

1. **Localizador de Escolas:** Painel dinâmico com busca multicritério e prefetching de dados.
2. **Ficha da Unidade com Edição Reativa:** Visualização individual, dados de contato e conta principal com **Optimistic Updates** para salvar alterações.
3. **Geração Documental em Memória (ExcelJS):** Gerador do **Demonstrativo Básico Individual** que manipula e injeta dados na aba `MEMÓRIA` do template oficial, removendo metadados brutos e fórmulas inválidas (`BASE!`, `XLOOKUP`).
4. **Dashboard Executivo-Operacional:** Indicadores em tempo real baseados em views calculadas no banco de dados.
5. **Geração em Lote dos 163 Demonstrativos:** Sistema em background com progresso visível que consolida e empacota demonstrativos de todas as unidades da CRE.
6. **Atualização Assistida de Diretores (Bulk Update - Marco 10B v2):** Mecanismo em 7 passos (Upload ➔ Validação ➔ Mapeamento Reativo ➔ Preview de Diff ➔ Confirmação ➔ Aplicação Transacional SQL ➔ Ações Recomendadas).
7. **Trilha de Auditoria & Segurança Transacional:** Alterações em lote executadas via **RPC (Security Definer) com SELECT FOR UPDATE** e logging automático em tabelas de auditoria (audit_logs).
8. **Extração Fiscal Multicanal (Python):** Módulo de processamento e extração de Notas Fiscais eletrônicas em lote.
9. **Central de Histórico de Gerações (/painel/historico):** Página de histórico de execuções com filtros avançados e visualizador JSONB de inconsistências cadastrais em tempo real.

---

## 🛠️ Stack Técnica do Projeto

O ecossistema tecnológico do projeto foi estruturado com foco em estabilidade, tipagem estrita e resiliência:

| Camada | Tecnologia | Propósito e Diferencial |
| :--- | :--- | :--- |
| **Frontend Core** | **React 19.2** + **Vite 7.3** | Interface declarativa, compilação otimizada e suporte a hooks reativos. |
| **Styling (CSS)** | **Vanilla CSS** + **Tailwind CSS v3** | Design Tokens para consistência, layouts HSL responsivos e transições suaves. |
| **UI Components** | **Radix UI** + **shadcn/ui** | Componentes semânticos com base de acessibilidade e design unificado. |
| **Micro-animações**| **Framer Motion 12** | Transições de painéis e micro-interações fluidas. |
| **Gerenciamento de Estado**| **TanStack React Query v5** | Gerenciamento de cache, prefetching em segundo plano e atualizações otimistas na interface. |
...
| **Motor de Planilhas** | **ExcelJS 4.4** + **Papaparse 5.5** | Manipulação de planilhas complexas com dynamic imports + parser de CSV robusto. |
| **Validação** | **Zod 4.4** | Governança de dados de entrada na UI com validação matemática modular (CNPJ Mod 11, INEP 8-dígitos). |
| **Suíte de Testes** | **Vitest 4.1** + **jsdom 29.1** | Testes de unidade e renderização com **123 testes** de cobertura. |
| **CI/CD** | **GitHub Actions** + **Vercel** | Integração contínua (Lint, TSC, Test, Build) e deploy automatizado. |

---

## ⚡ Estratégias de Performance & Governança

### 1. Navegação Otimizada (Query Prefetching)
Quando o usuário move o cursor do mouse (`onMouseEnter`) sobre uma linha de escola ou botão de ação, o sistema dispara silenciosamente o pré-carregamento (`prefetchQuery`) da view `vw_unidade_detalhe`. Nos ~200ms que o usuário leva para clicar, o dado já está no cache local. O carregamento de páginas e painéis laterais ocorre de forma imediata (zero tempo de espera perceptível).

### 2. Divisão de Bundle (Code-Splitting)
...
Para garantir governança absoluta dos dados antes de qualquer persistência, a aplicação utiliza esquemas do Zod com:
...
- Sanitização e verificação de e-mails institucionais.
...
└── tools/
    └── fiscal-extraction/   # Módulo Python independente para Extração Fiscal Multicanal
```
...
```bash
# Executar a suíte completa de testes unitários (123 testes)
npm test
...
O sistema possui um utilitário escrito em **Python 3** localizado em `tools/fiscal-extraction/` projetado para processar, validar e extrair dados financeiros de Notas Fiscais eletrônicas (NF-e) em lote.
...
# Validar campos extraídos contra os esquemas de compliance
...
Este repositório aplica regras de segurança e conformidade institucional. Ao trabalhar no código:
