-- Financial Publication Date Compatibility V4
--
-- Corrige a hipótese rígida da V3 de que a maturidade da parcela depende
-- exclusivamente de paymentOrderDate. O snapshot validado corrente do motor
-- pode trazer paymentInformedDate/paymentDate sem uma data de ordem distinta.
-- Para maturidade, qualquer uma das duas datas é evidência temporal válida;
-- na persistência, os campos continuam separados e nunca são inventados.
...
BEGIN
  ...
  SELECT
   ...
   WHERE r.program = 'PDDE BÁSICO'
     ...
     AND r.paid >= 0
     AND (r."paymentOrderDate" IS NOT NULL OR r."paymentDate" IS NOT NULL);
