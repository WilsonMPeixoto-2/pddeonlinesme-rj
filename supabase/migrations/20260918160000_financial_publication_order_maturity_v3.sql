-- Financial Publication Order Maturity V3
--
-- O valor pago informado e sua composição são fatos independentes da data.
-- A fonte FNDE usa "Data Ord. Pagamento"; portanto a maturidade da 1ª parcela
-- é validada pela ordem de pagamento, sem inventar data de pagamento/crédito.
--
-- A persistência continua separada:
--   paymentDate      -> data_pagamento
--   paymentOrderDate -> data_ordem_pagamento
...
BEGIN
  ...
  SELECT count(DISTINCT r.inep), min(r."paymentOrderDate"), max(r."paymentOrderDate")
